...
class FashionFormer(TwoStageDetector):
    ...
    def _show_result(self,
        ...
        if out_file is not None:
            show = False
        # draw bounding boxes
        category_info = self.imshow_det_bboxes(img, cv_img, bboxes, labels, segms, attrs, class_names=self.CLASSES, 
    ...
    def imshow_det_bboxes(self, img, cv_img, bboxes, labels, segms=None, attrs=None, class_names=None, score_thr=0,bbox_color='green', text_color='green',
        ...
        # for i, (bbox, label) in enumerate(zip(bboxes, labels)):
            ...
            # avg_color = get_average_color(img, bbox)

            # Map to closest HTML color
            # closest_color_name = get_closest_color(avg_color)
            # closest_color_name
            category_info.append([label, label_text, attr_text])
            # category_info.append([label, label_text, attr_text])  # Add label and empty attribute for now    
        ...
        for i, (bbox, label, old_bbox) in enumerate(zip(bboxes, labels, old_bboxes)):
            ...
            # for idx in attrs[i].nonzero()[0]:
            #     attr_text = attr_text +  self.attr_classes[idx] + '\n'
            category_info.append([label_text, attr_text, closest_html_color])  # Add label and empty attribute for now
            # label_text = f'{class_names[label]} {bbox[-1]*100:.0f}%'
            print(average_color)
            # avg_color = get_average_color(img, bbox_int)

            # Map to closest HTML color
            # closest_color_name = get_closest_color(avg_color)
            print(closest_html_color)
